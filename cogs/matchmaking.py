...
class JoinQueueView(discord.ui.View):
    ...
    async def _join(self, interaction: discord.Interaction, role: str) -> None:
        ...
        async with lock:
            # VIP lookup uses its own pool connection — fetch it before
            # entering the shared conn() block to avoid concurrent use
            # of the same asyncpg connection (which is not allowed).
            vip = await vip_data.get_active_vip(interaction.user.id)

            async with database.conn() as c:
                match_row = await db_get_match(c, self.mn)
                existing  = await _q(c, "SELECT 1 FROM mm_match_players WHERE match_number=$1 AND discord_id=$2", self.mn, uid)
...
class MMCog(commands.Cog):
    ...
    async def mm_elo(self, i: discord.Interaction, member: discord.Member | None = None):
        ...
        # VIP uses its own pool connection — fetch outside conn() block.
